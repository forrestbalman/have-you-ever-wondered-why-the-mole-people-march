<script>
    ...
    async function startTone() {
        ...
        bassLayer1 = {
            sounds: [new Tone.Player("/bass-layer-1.mp3"), new Tone.Player("/bass-layer-1.mp3")],
            gain: new Tone.Gain().toDestination(),
            timeout: null,
        };
        bassLayer2 = {
            sounds: [new Tone.Player("/bass-layer-2.mp3"), new Tone.Player("/bass-layer-2.mp3")],
            gain: new Tone.Gain().toDestination(),
            timeout: null,
        };
        bassLayer3 = {
            sounds: [new Tone.Player("/bass-layer-3.mp3"), new Tone.Player("/bass-layer-3.mp3")],
            gain: new Tone.Gain().toDestination(),
            timeout: null,
        };
    }
    ...
    function stepMaker(volume, taps, min, max, dmin, dmax) {
        ...
        if (!stepTimeout) delayMaker(volume, randomNumberInclusive(4, 6), "/bass-hit.mp3", dmin, dmax);
        //sets a timeout for the sounds to recur
        stepTimeout = setTimeout(
            () => {
                delayMaker(volume, taps, "/bass-hit.mp3", dmin, dmax);
    ...
    function dripMaker(volume, taps, min, max, dmin, dmax) {
        dripTimeout = setTimeout(
            () => {
                delayMaker(volume, taps, "/drip.mp3", dmin, dmax);
    ...
    function windMaker(volume, rate) {
        ...
        let wind = new Tone.Player("/wind.mp3").connect(panner);
    ...
    function progressSequence() {
        ...
        switch (sequence) {
            ...
            case 2:
                ...
                delayMaker(0.85, randomNumberInclusive(4, 6), "/bass-hit.mp3", 1100, 1250);
    ...
    function setMic() {
        ...
        if (micOpen) {
            ...
            micReverb = new Tone.Convolver("/MillsGreekTheater.wav").connect(micGain);
...
<div class="min-vh-100 bg-dark d-flex flex-column gap-3 justify-content-center align-items-center">
    ...
    {/if}
    {#if !allowAudio}
        <h1 class="display-2 text-light text-center">Have you ever wondered why the <span class="mole-container"><img class="svg-size mole" src="/mole.svg" alt="Mole icon" /></span> people march?</h1>
        <button
